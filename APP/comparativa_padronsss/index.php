...
<head>
	...
	<style type="text/css">
	    ...
	    /* Estilos para el panel de agrupamientos */
		#panelAgrupamientos {
...
<body>
	...
	<div class="container-fluid">
		<div class="x_panel">
			...
			<div class="float-start" role="group" aria-label="Acciones">
			    ...
			    <!-- Nuevo botón para agrupamientos -->
			    <button class="btn btn-secondary btn-sm" id="btnMostrarAgrupamientos">
			    ...
			    <button class="btn btn-danger btn-sm" id="btnBorrar">
			        <i class="far fa-check-square"></i> Borrar seleccionados
			    </button>
			    <button class="btn btn-danger btn-sm" id="btnInsertarPresentacion">
			        <i class="far fa-check-square"></i> Incluir en la presentacion actual 
			    </button>
			</div>
			<br><br><br><br>
			<!-- Navs -->
			...
			<div class="tab-content" id="tabMovimientosContent">
			  ...
			  <div class="tab-pane fade show active" id="filtros" role="tabpanel" aria-labelledby="filtros-tab">
			    ...
			    <div class="row">
			    	<br>
			    	<input type="hidden" name="filtroParentesco" id="filtroParentesco">
	...
	<div class="offcanvas offcanvas-end offcanvas-dark text-white" tabindex="-1" id="filtrosPanel">
	  ...
	  <div class="offcanvas-body">
	    ...
	    <div class="mb-3">
	      <label><b>Aprobados</b></label>
	      <select id="filtro_aprobado" class="form-control">
	        <option value="">Todos</option>
...
<script>
	...
	let id_lote = "<?=$id_lote;?>";
	// Variable para controlar el estado de los agrupamientos
	var agrupamientosVisibles = false;	
	...
	$(document).ready(function() {	
		...
		// Evento para mostrar/ocultar agrupamientos
	    $('#btnMostrarAgrupamientos').on('click', function() {
	        agrupamientosVisibles = !agrupamientosVisibles;
	        
	        if (agrupamientosVisibles) {
	            // Si no existen, crearlos
	            if (!$('#panelAgrupamientos').length) {
	                cargarAgrupamientos();
	            } else {
	                $('#panelAgrupamientos').show();
	            }
	            $(this).html('<i class="fas fa-chart-pie"></i> Ocultar resumen');
	        } else {
	            $('#panelAgrupamientos').hide();
	            $(this).html('<i class="fas fa-chart-pie"></i> Ver resumen');
	        }
	    });
	})
	...
	function genera_listado_con_filtros() {
	    ...
	    var table = $('#tabListadoGeneral').DataTable({
	        ...
	        ajax: {
	            ...
	            data: function(d) {
	                ...
	                d.id_lote = id_lote;	                
	                d.filtro_parentesco = $("#filtroParentesco").val();
	                d.filtro_aprobado = $("#filtro_aprobado").val();
	            },
	            dataSrc: function(json) {
	                // Mostrar los agrupamientos
	                mostrarAgrupamientos(json.agrupamientos,json.recordsTotal);
	                return json.data;
	            }
		    ...
		    { 
		        data: null,
		        name: 'index', // Columna para el índice
		        defaultContent: '',
		        orderable: false
		    },
		    { 
		    ...
		    { 
		        data: 'parentesco',
		        name: 'parentesco'  // Esta es la columna importante para el filtrado
		    },
		    { 
		        data: 'cuil',
		        name: 'cuil' 
		...
		columnDefs: [
		    {
		        targets: [0, 1], // Columnas de índice y checkbox
		        searchable: false,
		        orderable: false
		    },
		    {
		        targets: '_all', // Todas las columnas
		        className: 'text-center' // Alineación centrada
		    }
		],
	    ...
	    // Limpiar filtro cuando se hace clic en "Limpiar filtros"
	    $('#btnResetFiltros').off('click').on('click', function() {
	        var table = $('#tabListadoGeneral').DataTable();
	        table.ajax.params().filtro_parentesco = '';
	...
	function cargarAgrupamientos() {
	    // Obtener los agrupamientos del servidor
	    $.getJSON('ajax.php', {
	        parametro: "listado_previo_nov_y_padronsss",
	        solo_agrupamientos: true  // Nuevo parámetro para solo traer los agrupamientos
	    }, function(response) {
	        mostrarAgrupamientos(response.agrupamientos);
	    });
	}

	function mostrarAgrupamientos(agrupamientos, recordsTotal) {
	    var $panel = $('#panelAgrupamientos');
	    
	    if (!$panel.length) {
	        $panel = $(`
	            <div id="panelAgrupamientos" class="card mb-4" style="display: none;">
	                <div class="card-header d-flex justify-content-between align-items-center">
	                    <h5 class="mb-0">Resumen por categorías</h5>
	                    <button class="btn btn-sm btn-outline-secondary" id="btnResetFiltros">
	                        <i class="fas fa-times"></i> Limpiar filtros
	                    </button>
	                </div>
	                <div class="card-body" id="contenidoAgrupamientos"></div>
	            </div>
	        `).insertBefore('#tabListadoGeneral_wrapper');
	    }

	    var html = '<div class="row"><div class="col-md-4">';
	    html += '<h6>Parentesco</h6>';
	    html += '<ul class="list-group">';
	    
	    html += `<li class="list-group-item d-flex justify-content-between align-items-center">
	                <span class="badge-filter" data-field="parentesco" data-value="TODOS" 
	                      style="cursor:pointer; font-weight: bold;">
	                    TODOS
	                </span>
	                <span class="badge bg-secondary rounded-pill">${recordsTotal}</span>
	             </li>`;
	    
	    agrupamientos.parentesco.forEach(function(item) {
	        html += `<li class="list-group-item d-flex justify-content-between align-items-center">
	                    <span class="badge-filter" data-field="parentesco" data-value="${escapeHtml(item.parentesco)}" 
	                          style="cursor:pointer">
	                        ${escapeHtml(item.parentesco)}
	                    </span>
	    ...
	    html += '</ul></div></div>';
	    $('#contenidoAgrupamientos').html(html);
	    
	    $panel.off('click', '.badge-filter').on('click', '.badge-filter', function() {
	        var value = $(this).data('value');
	        var field = $(this).data('field');
	        if(value=="TODOS"){
	        	$("#filtroParentesco").val(''); 
	        }
	        else{
	        	console.log("valor filtro: "+value)
	        	$("#filtroParentesco").val(value); 
	        }
	        ...
	        // 1. Limpiar todos los filtros existentes
	        table.columns().search('');
	        table.search('');
	        
	        // 2. Aplicar filtro específico a la columna de parentesco
	        if (value !== 'TODOS') {
	            // Encontrar el índice de la columna parentesco
	            var columnIndex = table.column('parentesco:name').index();
	            
	            if (columnIndex !== undefined) {
	                // Filtrar exactamente por el valor de parentesco
	                table.column(columnIndex).search('^' + value + '$', true, false);
	            }
	        }
	        ...
	        if (value !== 'TODOS') {
	            mostrarFiltroActivo('Parentesco', value);
	        } else {
	            $('#filtroActivo').hide();
	        }
	    });
	...
	function mostrarFiltroActivo(categoria, valor) {
	    var $filtroActivo = $('#filtroActivo');
	    if (!$filtroActivo.length) {
	        $filtroActivo = $('<div id="filtroActivo" class="alert alert-info mb-3"></div>')
	            .insertAfter('#panelAgrupamientos');
	    }
	    
	    $filtroActivo.html(`
	        Filtro activo: <strong>${categoria}</strong> = <strong>${valor}</strong>
	        <button class="btn btn-sm btn-outline-danger float-end" id="btnQuitarFiltro">
	...
	$(document).on('click', '#btnBorrar', function() {
	    ...
	    if(resp){
		    ...
		    $.getJSON('ajax.php', {
		        parametro: "borrar_seleccionados",
		        registros: JSON.stringify(aprobados)
		    }, function(response) {
		        alert("Registros eliminado correctamente.");
		        $('#tabListadoGeneral').DataTable().ajax.reload(); // Recargar la tabla
		    });
	    }
	...
	$(document).on('click','#btnInsertarPresentacion',function(){
		var resp = confirm("Confirma ?");

		if(resp){
			$.getJSON('ajax.php', {
		        parametro: "insertarPresentacion",
		        id_lote: id_lote
		    }, function(response) {
		        alert("Registros eliminado correctamente.");
		        $('#tabListadoGeneral').DataTable().ajax.reload(); // Recargar la tabla
