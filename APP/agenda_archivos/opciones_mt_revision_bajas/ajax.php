...
switch ($parametro) {
	...
	case 'TraerProcesados':
		...
		$sql="SELECT id, 
			...
			WHERE proceso='opciones_mt_revision_bajas'
...
function graba_lote_y_cierra($konta,$fdesde,$fhasta,$id_usuario){
	$inserta="INSERT INTO ".N_BASE_HISTORICOS.".lotes(lote,archivo,descripcion,cant_registros,proceso,id_usuario,usuario,exp_padron)
				VALUES('$fdesde','$fhasta',CONCAT('$fdesde',' al ','$fhasta'),'$konta','opciones_mt_revision_bajas',$id_usuario,'$usu',1)"; 
	...
	mysql_query("UPDATE ".N_BASE_HISTORICOS.".opciones_mt_revision_bajas SET id_lote=$id_lote WHERE id_lote IS NULL ") or die(mysql_error());	
	return $id_lote;
}
...
function insertar($konta,$input){
	...
	$sql = "INSERT INTO ".N_BASE_HISTORICOS.".opciones_mt_revision_bajas(tipo_mt,nro_formulario,cuil_titular,ayn,fecha_vigencia,tel_contacto,email,
...
function insertar_en_aai($id_lote,$id_user){
	...
	$query="CALL ".N_BASE_PADRON.".aai_insertar_altas_mt($id_lote, $id_user);";
