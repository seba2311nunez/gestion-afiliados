...
switch ($parametro) {
	...
	case 'valida_archivo_a_procesar':
		...
		$sql="SELECT * FROM $base_historicos.lotes WHERE descripcion='$periodo' AND proceso='expedientes_sss'";
	...
	case 'trabajar_archivo':
        ...
        if (move_uploaded_file($_FILES['archivo']['tmp_name'], $nombre_archivo_temporal)) {
			...
			$contador="SELECT COUNT(*) as contador FROM $base_historicos.expedientes_sss WHERE id_lote=0";
...
function graba_lote_y_cierra($konta,$periodo,$nombre_archivo,$id_usuario){
	$inserta="INSERT INTO ".N_BASE_HISTORICOS.".lotes (archivo,descripcion,cant_registros,proceso,id_usuario,clave_agenda)
				VALUES('$nombre_archivo','$periodo','$konta','expedientes_sss',$id_usuario,'expedientes_sss_$periodo')"; 
	...
	mysql_query("UPDATE ".N_BASE_HISTORICOS.".expedientes_sss SET id_lote=$id_lote WHERE id_lote=0") or die(mysql_error());

	#mysql_query("CALL ".N_BASE_HISTORICOS.".`EXP_proceso_completo`($id_lote)");#Sera necesario?
...
function insertar($konta,$input,$periodo,$periodo_formateado){
	...
	list($rnos,$cuil,$ayn,$mov,$mov_detalle,$fecha_vigencia,$nro_exp,$anio_exp,$tipo_dispo,$dispo)=explode("|",$input);
	
	
	$sql="
	INSERT INTO ".N_BASE_HISTORICOS.".expedientes_sss (`id_lote`,`rnos`,`cuil`,`ayn`,`mov`,`mov_detalle`,`fecha_vigencia`,`nro_exp`,`anio_exp`,`tipo_dispo`,`dispo`)
	VALUES	(0,'$rnos','$cuil','$ayn','$mov','$mov_detalle','$fecha_vigencia','$nro_exp','$anio_exp','$tipo_dispo','$dispo')
	";
