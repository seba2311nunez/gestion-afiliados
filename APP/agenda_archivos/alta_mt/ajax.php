...
switch ($parametro) {
	...
	case 'valida_archivo_a_procesar':
		...
		$sql="SELECT * FROM $base_historicos.lotes WHERE descripcion='$periodo' AND proceso='alta_mt'";
	...
	case 'trabajar_archivo':
		...
		if($_SERVER['DOCUMENT_ROOT']=="/var/www/".DOMINIO){
			$archivo_send =  $_SERVER['DOCUMENT_ROOT']."/padron/APP/agenda_archivos/alta_mt/archivos/";
		}
		else{
			$archivo_send = $_SERVER['DOCUMENT_ROOT']."/padron/APP/agenda_archivos/alta_mt/archivos/";
		}
...
function insertar($konta,$input,$periodo,$periodo_formateado){
	...
	$sql=" 
	INSERT INTO ".N_BASE_HISTORICOS.".altas_super_mt (nro_formulario,cuil,ayn,calle,numero,piso,dto,cp,localidad,provincia,fecha_eleccion,periodo_alta,cod_movimiento,cuil_nuevo,categoria_mt,periodo,id_lote)
	VALUES ('$a','$b','$c','$d','$e','$f','$g','$h','$i','$j','$k','$l','$m','$n','$o','$periodo_formateado','$periodo_formateado')
	";
...
function graba_lote_y_cierra($konta,$periodo,$nombre_archivo,$id_usuario){
	$inserta="INSERT INTO ".N_BASE_HISTORICOS.".lotes(archivo,descripcion,cant_registros,proceso,id_usuario,clave_agenda)
				VALUES('$nombre_archivo','$periodo','$konta','alta_mt',1,'alta_mt_$periodo')"; 
	...
	mysql_query("UPDATE ".N_BASE_HISTORICOS.".altas_super_mt SET id_lote=$id_lote WHERE id_lote=periodo") or die(mysql_error().$sql);

	mysql_query("UPDATE ".N_BASE_HISTORICOS.".agenda SET procesado=1 WHERE clave='alta_mt_$periodo'") or die(mysql_error().$sql);
	
	mysql_query("UPDATE ".N_BASE_HISTORICOS.".altas_super_mt SET cuil=MID(cuil,1,11) WHERE id_lote=$id_lote AND cuil like '%.0'");
	return $id_lote;
}
