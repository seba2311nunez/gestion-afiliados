...
<head>
	<meta charset="UTF-8">
	<title>ALTAS RG | Opciones revision SSS</title>
	<!-- Jquery -->
...
<body>
	...
	<div class="container" id='container_form'>
		...
		<div class="row" style="width: 600px;">
			<div class="x_panel">
				<div class="tituloDiv">
					Subir un nuevo archivo de <b>traspasos | Altas RG </b>
				</div>
				<!-- Form de carga -->
				...
				<div class="row">
					<div style="width: 99%; padding: 20px;">
						...
						<div style="margin: 10px 0;">
							<button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#ModalResumenAltas">
								<i class="fas fa-chart-bar"></i> Resumen por mes
							</button>
						</div>

						<!-- Modal Resumen Altas -->
						<div class="modal fade" id="ModalResumenAltas" tabindex="-1" role="dialog" aria-labelledby="ModalResumenAltasLabel">
							<div class="modal-dialog modal-lg" role="document">
								<div class="modal-content">
									<div class="modal-header">
										<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
										<h4 class="modal-title" id="ModalResumenAltasLabel">Resumen Altas RG</h4>
									</div>
									<div class="modal-body">
										<div class="row" style="margin-bottom: 15px;">
											<div class="col-sm-6">
												<label>Mes</label>
												<select id="periodo_resumen" class="form-control"></select>
											</div>
											<div class="col-sm-6" style="padding-top: 25px;">
												<button type="button" class="btn btn-success" id="btnBuscarResumen">
										...
										<div class="table-responsive">
											<table class="table table-bordered" id="tabResumenAltas">
												<thead>
													<tr>
														<th>Periodo</th>
														<th>Altas mensuales</th>
														<th>Altas diarias</th>
														<th>Diferencia</th>
													</tr>
												</thead>
										...
										<div class="alert alert-warning" id="alertResumenAltas" style="display:none; margin-top:10px;"></div>
									</div>
...
<script>
	...
	var error_tipo_archivo = "No es un archivo 'Opciones RG revision' ";

	const DOMINIO = "<?echo DOMINIO;?>";
	const INST_RNOS = "<?echo INST_RNOS;?>";
	var dtResumenAltas = null;
	      ...
	      if(data[0]['error']){
	        ...
	        if(data.length>0){
	          for(var i=0; i<=data.length-1 ;i++){
	            ...
	            // Mostrar YYYY-MM (sin -01)
	            var periodoVisual = (data[i]['periodo1'] || '').toString().substring(0,7);

	            $('#periodo').append(
	              "<option value='"+data[i]['primer_dia']+"'>"+periodoVisual+"</option>"
	            );

	            if($('#periodo_resumen').length){
	              $('#periodo_resumen').append(
	                "<option value='"+data[i]['primer_dia']+"'>"+periodoVisual+"</option>"
	              );
	            }
	          }
	...
	// --- Resumen Altas RG por mes ---
	function InicializarTablaResumen(){
		if(dtResumenAltas){ return; }
		dtResumenAltas = $('#tabResumenAltas').DataTable({
			dom: 'Blrtip',
			buttons: [
	...
	function CargarResumenAltas(periodo){
		InicializarTablaResumen();
		$('#alertResumenAltas').hide().text('');

		$.getJSON('ajax.php',
			{parametro:'TraerResumenAltasRg', periodo: periodo},
			function(data){
				dtResumenAltas.clear();

				if(!data || !data.length || data[0]['error']){
					var msg = (periodo && periodo!='') ? 'No hay resultados para el periodo seleccionado.' : 'No hay resultados en los últimos 6 meses.';
					$('#alertResumenAltas').show().text(msg);
					dtResumenAltas.draw();
					return;
				}

				for(var i=0; i<=data.length-1; i++){
					dtResumenAltas.row.add([
						(data[i]['periodo'] || '').toString().substring(0,7), // YYYY-MM
						data[i]['altas_mensuales'],
						data[i]['altas_diarias'],
						data[i]['dif']
					]);
				}
				dtResumenAltas.draw();
	...
	$(document).on('click','#btnBuscarResumen',function(){
		var per = $('#periodo_resumen').val();
		if(!per){
			$('#alertResumenAltas').show().text('Seleccione un mes.');
			return;
		}
		CargarResumenAltas(per);
	});

	$('#ModalResumenAltas').on('hidden.bs.modal', function(){
		$('#alertResumenAltas').hide().text('');
		if(dtResumenAltas){ dtResumenAltas.clear().draw(); }
	});

	$('#ModalResumenAltas').on('shown.bs.modal', function () {
		InicializarTablaResumen();
		if(dtResumenAltas){ dtResumenAltas.columns.adjust(); }
		CargarResumenAltas('');
		var per = $('#periodo_resumen').val();
		// si querés, después podés apretar Buscar para un mes específico
	});
