...
switch ($parametro) {
	...
	case 'TraerProcesados':
		...
		$sql="SELECT l.id, 
					...
					LEFT JOIN $base_historicos.`opciones_rg_revision_altas` a ON l.id=a.id_lote 
					WHERE proceso LIKE 'opciones_rg_revision_altas' or proceso LIKE 'opciones_rg_revision'
					GROUP BY 1
					ORDER BY lote DESC,archivo DESC 
					";
	...
	case 'valida_archivo_a_procesar':
		...
		$sql = "SELECT COUNT(*) AS q
					...
					WHERE proceso='opciones_rg_revision'
...
function graba_lote_y_cierra($konta,$fdesde,$fhasta,$id_usuario,$usu){
	...
	$inserta="INSERT INTO ".N_BASE_HISTORICOS.".lotes(lote,archivo,descripcion,cant_registros,proceso,id_usuario,usuario,exp_padron)
				VALUES('$fdesde','$fhasta',CONCAT('$fdesde',' al ','$fhasta'),'$konta','opciones_rg_revision_altas',$id_usuario,'$usu','$exp_padron')"; 
	mysql_query($inserta) or die(mysql_error().$inserta);
	$id_lote = mysql_insert_id();
	mysql_query("UPDATE ".N_BASE_HISTORICOS.".opciones_rg_revision_altas SET id_lote=$id_lote WHERE id_lote IS NULL ") or die(mysql_error());
...
function insertar($konta,$input){
	...
	$sql = "INSERT INTO ".N_BASE_HISTORICOS.".opciones_rg_revision_altas(nro_formulario,cuil_titular,ayn,fecha_vigencia,telefono,email,
