...
switch ($parametro) {
	...
	case 'valida_archivo_a_procesar':
		...
		$sql="SELECT * FROM $base_historicos.lotes WHERE descripcion='$periodo' AND proceso='baja_b15'";
...
function insertar($konta,$input,$periodo,$periodo_formateado){
	...
	list($a,$b,$c,$d,$e,$f,$g,$h,$i,$j,$k,$l,$m)=explode("|",$input);
		
	$sql=" 
		INSERT INTO ".N_BASE_HISTORICOS.".bajas_super_b15 (cuit_titular,obra_social,periodo_baja,cuit_nuevo,ayn,calle,num,piso,dpto,localidad,codpos,provincia,categoria,periodo,id_lote)
		VALUES ('$a','$b','$c','$d','$e','$f','$g','$h','$i','$j','$k','$l','$m','$periodo_formateado','$periodo_formateado')
	";
...
function graba_lote_y_cierra($konta,$periodo,$nombre_archivo,$id_usuario){
	$inserta="INSERT INTO ".N_BASE_HISTORICOS.".lotes(archivo,descripcion,cant_registros,proceso,id_usuario,clave_agenda)
				VALUES('$nombre_archivo','$periodo','$konta','baja_b15',1,'baja_b15_$periodo')"; 
	...
	mysql_query("UPDATE ".N_BASE_HISTORICOS.".bajas_super_b15 SET id_lote=$id_lote WHERE id_lote=periodo COLLATE latin1_general_ci") or die(mysql_error());

	mysql_query("UPDATE ".N_BASE_HISTORICOS.".bajas_super_b15 bsb
			SET fecha_super=ADDDATE(CONCAT(MID(periodo_baja,1,4),'-',MID(periodo_baja,5,2),'-01'),INTERVAL 1 MONTH)");
