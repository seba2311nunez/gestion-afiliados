...
switch ($parametro) {
	...
	case 'valida_archivo_a_procesar':
		...
		$sql="SELECT * FROM $bd.lotes WHERE descripcion='$periodo' AND proceso='baja_mt'";
	...
	case 'trabajar_archivo':
		...
		if($_SERVER['DOCUMENT_ROOT']=="/var/www/".DOMINIO){
			$archivo_send =  $_SERVER['DOCUMENT_ROOT']."/padron/APP/agenda_archivos/baja_mt/archivos/";
		}
		else{
			$archivo_send = $_SERVER['DOCUMENT_ROOT']."/padron/APP/agenda_archivos/baja_mt/archivos/";
		}
		...
		if($copiado==false){
			...
			echo "Error moviendo el archivo ".$archivo_send;
			...
			$contador="SELECT COUNT(*) as contador FROM $bd.altas_super WHERE periodo_recibido='$periodo_formateado'";
...
function insertar($konta,$input,$periodo,$periodo_formateado){
	...
	$sql=" 
		INSERT INTO ".N_BASE_HISTORICOS.".bajas_super_mt (nro_formulario,cuil,ayn,calle,numero,piso,dto,cp,localidad,provincia,cuit,fecha_eleccion,tipo,periodo_baja,categoria_mt,periodo,id_lote)
...
function graba_lote_y_cierra($konta,$periodo,$nombre_archivo,$id_usuario){
	$inserta="INSERT INTO ".N_BASE_HISTORICOS.".lotes(archivo,descripcion,cant_registros,proceso,id_usuario,clave_agenda)
				VALUES('$nombre_archivo','$periodo','$konta','baja_mt',1,'baja_mt_$periodo')"; 
	...
	mysql_query("UPDATE ".N_BASE_HISTORICOS.".bajas_super_mt SET id_lote=$id_lote WHERE id_lote=periodo") or die(mysql_error());
