...
switch ($parametro) {
	...
	case 'valida_archivo_a_procesar':
		...
		$sql="SELECT * FROM ".N_BASE_HISTORICOS.".lotes WHERE descripcion='$periodo' AND proceso='bajas_mt_sss'";
	...
	case 'trabajar_archivo':
        ...
        if (move_uploaded_file($_FILES['archivo']['tmp_name'], $nombre_archivo_temporal)) {
			...
			/*Test*/
			$contador="SELECT COUNT(*) as contador FROM ".N_BASE_HISTORICOS.".bajas_mt_sss WHERE id_lote=0";
...
function graba_lote_y_cierra($konta,$periodo,$nombre_archivo,$id_usuario){
	$inserta="INSERT INTO ".N_BASE_HISTORICOS.".lotes (archivo,descripcion,cant_registros,proceso,id_usuario,clave_agenda)
				VALUES('$nombre_archivo','$periodo','$konta','bajas_mt_sss',$id_usuario,'bajas_mt_sss_$periodo')"; 
	...
	mysql_query("UPDATE ".N_BASE_HISTORICOS.".bajas_mt_sss SET id_lote=$id_lote WHERE id_lote=0") or die(mysql_error());
...
function insertar($konta,$input,$periodo,$periodo_formateado){
	...
	list($nro_formulario,$rnos,$cuil_titular,
						$ayn,$tel_contacto,$telefono_laboral,$calle,$numero,$piso,$departamento,$codigo_postal,$localidad,$provincia,$os_origen,$fecha_eleccion,$periodo,$fecha_entrega,$mail,$categoria_mtr,$categoria_aut,$movimiento_afip)=explode("|",$input);
	
	
	$sql="
	INSERT INTO ".N_BASE_HISTORICOS.".bajas_mt_sss (id_lote,nro_formulario,rnos,cuil_titular,ayn,tel_contacto,tel_laboral,
