...
case 'lst_afiliados_presentacion_ssp': 
	...
	$filtro_incapacidad = isset($_GET['filtro_incapacidad']) ? mysql_real_escape_string($_GET['filtro_incapacidad']) : '';
	...
	if ($search != '') {
		$where_parts[] = "(nd LIKE '%$search%' 
			OR cuil LIKE '%$search%' 
			OR ayn LIKE '%$search%' 
			OR tbt LIKE '%$search%' 
			OR errores LIKE '%$search%' 
			OR rechazos LIKE '%$search%')";
	}

	if ($filtro_incapacidad != '') {

		$where_parts[] = "(
			incapacidad = '$filtro_incapacidad'
			OR EXISTS (
				SELECT 1
				FROM $base_padron.afiliados af_fila
				JOIN $base_padron.afiliados af_tit 
					ON af_tit.id = IF(af_fila.id_titular IS NULL OR af_fila.id_titular = 0, af_fila.id, af_fila.id_titular)
				JOIN $base_padron.afiliados af_grupo 
					ON (af_grupo.id_titular = af_tit.id OR af_grupo.id = af_tit.id)
				WHERE af_fila.id_persona = tmp.id_persona
					AND af_grupo.incapacidad = '$filtro_incapacidad'
			)
		)";
	}
	...
	$total_rs = mysql_query("SELECT COUNT(*) AS total FROM $base_padron.tmp_afiliados_novedades_mostrar tmp $where");
	$total_row = mysql_fetch_assoc($total_rs);
	$recordsTotal = intval($total_row['total']);

	$sql = "SELECT * FROM $base_padron.tmp_afiliados_novedades_mostrar tmp $where LIMIT $offset, $limit";
	$result = mysql_query($sql) or die(mysql_error().'<br>'.$sql);
	...
	case 'resumen_tbt_y_errores':
		...
		$sql_disc = "SELECT COUNT(DISTINCT a2.id) AS total
			FROM $base_historicos.novedades_exportables ne
			JOIN $base_padron.persona p ON ne.id_persona = p.id
			JOIN $base_padron.afiliados a ON p.id = a.id_persona
			JOIN $base_padron.afiliados a_tit ON a_tit.id = IF(a.id_titular IS NULL OR a.id_titular = 0, a.id, a.id_titular)
			JOIN $base_padron.afiliados a2 ON (a2.id_titular = a_tit.id OR a2.id = a_tit.id)
			WHERE ne.id_lote = $id_lote
				AND a2.incapacidad = '01'";
		$rs_disc = mysql_query($sql_disc) or die(mysql_error()."<br>".$sql_disc);
		$row_disc = mysql_fetch_assoc($rs_disc);
		$resumen_discapacitados = intval($row_disc['total']);

		echo json_encode(array(
		    'resumen_tbt' => $resumen_tbt,
		    'resumen_errores' => $resumen_errores,
		    'resumen_discapacitados' => $resumen_discapacitados
		));
	...
	case 'lst_discapacitados_presentacion_ssp':
		...
		if ($search != '') {
			$where_search = " AND (
				p.cuil LIKE '%$search%'
				OR p2.cuil LIKE '%$search%'
				OR p2.nd LIKE '%$search%'
				OR p2.apellido LIKE '%$search%'
				OR p2.nombre LIKE '%$search%'
				OR CONCAT(p2.apellido, ' ', p2.nombre) LIKE '%$search%'
				OR pa.parentesco LIKE '%$search%'
				OR d.convenio LIKE '%$search%'
				OR d.convenio_real LIKE '%$search%'
				OR ne.tipo_mov LIKE '%$search%'
			)";
		}
		...
		$from = "
			...
			JOIN $base_padron.afiliados a ON p.id = a.id_persona
			JOIN $base_padron.afiliados a_tit ON a_tit.id = IF(a.id_titular IS NULL OR a.id_titular = 0, a.id, a.id_titular)
			JOIN $base_padron.persona p_tit ON p_tit.id = a_tit.id_persona
			JOIN $base_padron.afiliados a2 ON (a2.id_titular = a_tit.id OR a2.id = a_tit.id)
			JOIN $base_padron.persona p2 ON a2.id_persona = p2.id
			LEFT JOIN $base_padron.parentesco pa ON pa.id = a2.id_parentesco
			LEFT JOIN $base_padron.desreguladoras d ON d.id = a2.id_desreguladora
			WHERE ne.id_lote = $id_lote
				AND a2.incapacidad = '01'
		";

		$sql_total = "SELECT COUNT(*) AS total FROM (SELECT DISTINCT ne.id AS id_expo, a2.id AS id_afiliado $from $where_search) x";
		$rs_total = mysql_query($sql_total) or die(mysql_error()."<br>".$sql_total);
		$row_total = mysql_fetch_assoc($rs_total);
		$recordsTotal = intval($row_total['total']);

		$sql = "
			SELECT DISTINCT
				ne.id AS id_expo,
				ne.tipo_mov,
				p.cuil AS cuil_presentado,
				a_tit.id AS id_titular,
				a2.id AS id_afiliado,
				p2.id AS id_persona,
				p_tit.cuil AS cuil_titular,
				COALESCE(pa.parentesco, '') AS parentesco,
				p2.cuil,
				p2.nd,
				CONCAT(p2.apellido, ' ', p2.nombre) AS ayn,
				p2.sexo,
				p2.fn,
				a2.incapacidad,
				COALESCE(d.convenio_real, d.convenio, '') AS desreguladora
			$from
			$where_search
			ORDER BY p_tit.cuil, pa.id, p2.apellido, p2.nombre
			LIMIT $start, $length
		";
	...
	case 'exportar_discapacitados_excel':
		...
		$id_lote = isset($_GET['id_lote']) ? intval($_GET['id_lote']) : 0;
		$filename = "discapacitados_presentacion_lote_" . $id_lote . ".xlsx";
		...
		$objPHPExcel = new PHPExcel();
		$sheet = $objPHPExcel->getActiveSheet();
		$sheet->setTitle("Discapacitados");

		$headers = array('#', 'CUIL presentado', 'Tipo mov. presentado', 'CUIL titular', 'Parentesco', 'CUIL discapacitado', 'DNI', 'Apellido y nombre', 'Sexo', 'Fecha nacimiento', 'Incapacidad', 'Gerenciadora');
		$col = 0;
		foreach ($headers as $h) {
			$sheet->setCellValueByColumnAndRow($col++, 1, $h);
		}
		...
		while ($row = mysql_fetch_assoc($result)) {
			$col = 0;
			$sheet->setCellValueByColumnAndRow($col++, $rowIndex, $i++);
			$sheet->setCellValueByColumnAndRow($col++, $rowIndex, $row['cuil_presentado']);
			$sheet->setCellValueByColumnAndRow($col++, $rowIndex, $row['tipo_mov']);
			$sheet->setCellValueByColumnAndRow($col++, $rowIndex, $row['cuil_titular']);
			$sheet->setCellValueByColumnAndRow($col++, $rowIndex, $row['parentesco']);
			$sheet->setCellValueByColumnAndRow($col++, $rowIndex, $row['cuil']);
	...
	case 'procesar_errores':
			...
			if(isset($_FILES['file_errores']) && $_FILES['file_errores']['error']==0) {
				...
				$archivo_send = $_SERVER['DOCUMENT_ROOT']."/padron/APP/genera_novedades/aceptados/";
