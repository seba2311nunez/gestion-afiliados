					...
					<div class="col-xs-12 " style="width: 100%;">
						...
						<nav>
							<div class="nav nav-tabs nav-fill" id="nav-tab" role="tablist">
								...
								<a class="nav-item nav-link" id="nav-dt-adicionales-tab" data-toggle="tab" href="#nav-dt-adicionales" role="tab" aria-controls="nav-dt-adicionales" aria-selected="false">Informacion complementaria del afiliado</a>
							</div>
						</nav>
						<div class="tab-content py-3 px-3 px-sm-0" id="nav-tabContent">
		                	...
		                	<!-- Datos Adicionales -->
							<div class="tab-pane fade" id="nav-dt-adicionales" role="tabpanel" aria-labelledby="nav-dt-adicionales-tab">
								<input type="hidden" name="formDatosAdicionales" value="0">

								<div>
									<table id="tabDatosAdicionales" class="table table-hover table-dark tabla-sm table-striped" style="font-size: 11px; width: 90%; margin: 20px;">
										<thead>
											<tr>
												<th>#</th>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <button type="button" class="btn-close" data-dismiss="modal" data-bs-dismiss="modal" aria-label="Close">X</button>
            </div>
            <div class="modal-body">
                <table class="table table-striped table-dark">
                    ...
                    <tbody id="historialContent">
                        <!-- Aquí se cargarán los registros del historial -->
                    </tbody>
                </table>
            </div>
            <div class="modal-footer">
		...
		$(function(){
	      ...
	      templateSelection: formatRepoSelection
			...
			$('#tabGrupoFamiliar').on('click','.verAfiliado',function(){
				...
				$.getJSON(ajax_url,
					{ parametro: "lst_datos_aficionales", id_afiliado: id_afiliado },						       				
					function(data){ 

						console.log("Info de lst_datos_aficionales: "+data)
						
						$("#tabDatosAdicionales tbody").html("");

						if(data[0]['estado']=="ERROR - NO se encontraron resultados"){
							$("#tabDatosAdicionales tbody").append("<tr>"																
								+"<td colspan=6>No se subieron archivos</td>"
								+"</tr>") ;	
						}
						else{
							...
							for(var i=0; i<=data.length-1 ;i++){
								...
								$("#tabDatosAdicionales tbody").append("<tr>"																
									...
									+"<td><input type='date' class='p_fecha_inicio' data-fila='"+i+"' value='"+data[i]['fecha_inicio']+"' /></td>"
									+"<td>"+data[i]['copciones']+"</td>"									
									+"<td><input type='date' class='p_fecha_limite' data-fila='"+i+"' value='"+data[i]['fecha_limite']+"' /></td>"
									+"<td><input type='number' step='0.01' class='p_fecha_limite' data-fila='"+i+"' value='"+data[i]['cprecio']+"'  placeholder='Ej: 1452,01' /></td>"
									+"<td><textarea class='form-control p_observacion' data-fila='"+i+"'>"+data[i]['cobservaciones']+"</textarea></td>"
									+"<td>"+data[i]['cdocumentacion']+"</td>"
									//+"<td>"+data[i]['usuario']+"</td>"
									//+"<td>"+data[i]['fechador_carga']+"</td>"											
									+"<td>"
										+"<a class='btn btn-warning btn-da_modificar' data-fila='"+i+"' data-id_registro='"+data[i]['id']+"' >M</a>"
										//+"<a class='btn btn-danger btn-da_eliminar' data-fila='"+i+"' data-id_registro='"+data[i]['id']+"'>E</a>"
										+"<a class='btn btn-info btn-ver_historial' data-id_registro='"+data[i]['id']+"'>H</a>"
									+"</td>"
									+"</tr>") ;		
							}	
						...
						// Función para el botón de modificar
						    $(".btn-da_modificar").click(function() {
						        // Obtener el ID de registro y la fila
						        let idRegistro = $(this).data("id_registro");
						        let fila = $(this).data("fila");

						        // Obtener los valores de los inputs de la fila
						        let fechaInicio = $(`input.p_fecha_inicio[data-fila='${fila}']`).val();
						        let fechaLimite = $(`input.p_fecha_limite[data-fila='${fila}']`).val();
						        let precio = $(`input[type='number'][data-fila='${fila}']`).val();
						        let observacion = $(`textarea.p_observacion[data-fila='${fila}']`).val();

						        //console.log("Fecha de inicio a modificar: "+fechaInicio); return false;
						    ...
						    $(".btn-da_eliminar").click(function() {
						        ...
						        // Enviar la solicitud de eliminación por AJAX
						        $.ajax({
						            url: 'ajax.php',
						            type: 'POST',
						            data: {
						            ...
						            success: function(response) {
						                if (response === 'ok' || response === 'true') {
						                    ...
						                    location.reload(); // Recargar la página o eliminar la fila de la tabla
						                } else {
						    ...
						    // Función para el botón de ver historial
						    $(".btn-ver_historial").click(function() {
						        let idRegistro = $(this).data("id_registro");

						        // Llamada AJAX para obtener el historial
						        $.ajax({
						            url: 'ajax.php',
						            type: 'POST',
						            data: {
						                parametro: 'dt_adicional_ver_historial',
						                id: idRegistro
						            },
						            dataType: 'json',
						            success: function(response) {
									    $("#historialContent").empty(); // Limpia cualquier contenido previo en el tbody de la tabla

									    if (Array.isArray(response) && response.length > 0) {
									        response.forEach(function(item) {
									            $("#historialContent").append(`
									                <tr>
									                	<td>${item.id_propio}</td>
									        ...
									        $("#historialContent").append("<tr><td colspan='8'>No hay historial disponible</td></tr>");
									    }

									    // Muestra el modal si estás usando uno
	...
	if(!fecha){
				...
				if (r == true) {
					...
					if($('input[name=formDatosPersonales]').val()==1){
						...
						if($('input[name=desreguladora_ant]').val() != $('select[name=desreguladora]').val()){
							$('#observacion_desreguladora').val(prompt('Observacion por cambio de Capita'));
						}
				...
				$.ajax({
					...
					if(propio == 1){
						if(!cod_filial){
							...
							if (respuesta != null) {
								let url = "../carnets/imprimir_carnets.php?sl=678&fv="+respuesta+"&cuil="+cuil;
								window.open(url);
							}	
						}
					}else{
		...
		function VGP(data,id_af_consultado){
			...
			for(var i=0; i<=data.length-1 ;i++){
				...
				tr_estado = item_sincronizar = badge_sincronizar = estados_item = documentacion_item = documentacion_item_surge = cambios_item = item_grupo_familiar = item_desvincular_familiar = credenciales = li_modal_crono = td_bgcolor_sind = td_consultado_style = td_nro_sind = td_patologias = "";
