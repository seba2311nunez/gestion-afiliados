	...
	if(!fecha){
			...
			$('#btnGuardarCambios').on('click',function(){
				...
				if (r == true) {
					...
					if($('input[name=formDatosAfiliacion]').val()==1){
						...
						if(tipo_perfil != 'admin' && tipo_perfil != 'upd_dto_personales'){//Esta validacion queda obsoleta
						...
						if(!cod_filial){
							...
							if (respuesta != null) {
								...
								if(tit_tbt == "RG"){
									...
									console.log('esMayorADiferenciaDe3Meses: '+esMayorADiferenciaDe3Meses(ult_per));
									...
									if(!tit_empresa){
										...
										window.open(url);
									}else if (esMayorADiferenciaDe3Meses(ult_per)){
										empresa = prompt('El afiliado no tiene una empresa declarada en los ultimos 3 meses. Cargue el nombre de la empresa.');
										if(empresa){
											url = "../carnets/imprimir_carnets.php?sl=678&fv="+respuesta+"&cuil="+cuil+"&empresa="+empresa;
										}
										window.open(url);
									...
									if(tit_empresa && !esMayorADiferenciaDe3Meses(ult_per)){
		...
		function VGP(data,id_af_consultado){
			...
			for(var i=0; i<=data.length-1 ;i++){
				...
				tr_estado = item_sincronizar = badge_sincronizar = estados_item = documentacion_item = documentacion_item_nuevo = documentacion_item_surge =  cambios_item = item_grupo_familiar = item_desvincular_familiar = credenciales = li_modal_crono = td_bgcolor_sind = td_consultado_style = td_patologias = td_nro_sind = item_ver_datos = div_dropdown_menu = "";



				if(tipo_perfil !='consulta_externo'){ 	

					item_ver_datos = "<a class='dropdown-item verAfiliado' ' style='cursor:pointer;' data-id_afiliado='"+data[i]['id']+"' data-s_nom_afil='"+data[i]['ayn_persona']+"' data-t_paren='"+data[i]['parentesco']+"' data-cuil='"+data[i]['cuil']+"' data-nd='"+data[i]['nd']+"' >Ver Datos</a>";

					var institucion_documentacion = N_BASE.toLowerCase();

					if(institucion_documentacion === "osemm"){
						documentacion_item_nuevo = "<a class='dropdown-item' style='cursor:pointer;' href='../docu_afiliados/?id_afiliado="+data[i]['id']+"&dni="+data[i]['nd']+"' target='_blank' >"
						+" Documentacion S3</a>";
					}else if(institucion_documentacion === "ospilm"){
						documentacion_item_nuevo = "<a class='dropdown-item' style='cursor:pointer;' href='../doc_export/docu_afiliados/?id_afiliado="+data[i]['id']+"&dni="+data[i]['nd']+"' target='_blank' >"
						+" Documentacion S3</a>";
					}else{
						...
						if(institucion_documentacion !== "ospm"){
				...
				div_dropdown_menu = item_sincronizar
					+item_ver_datos
					+estados_item
					+cambios_item
					+li_modal_crono
				...
				if(tipo_perfil == "equipo_interdisciplinario"){
					div_dropdown_menu = documentacion_item + documentacion_item_surge + documentacion_item_nuevo;

				}

				
				$("#tabGrupoFamiliar tbody").append("<tr "+tr_estado+" data-id_persona="+data[i]['id_persona']+">"																
					+"<td>"+data[i]['id']+"</td>"
					+"<td>"
					+"<div class='dropdown'>"
		...
		function esMayorADiferenciaDe3Meses(valor) {
		  const ahora = new Date();
		  const anioActual = ahora.getFullYear();
		  const mesActual = ahora.getMonth() + 1;

		  const anio = 2000 + parseInt(valor.substring(0, 2), 10);
		  const mes = parseInt(valor.substring(2, 4), 10);

		  // Convertimos ambas fechas a "cantidad total de meses" desde año 0
		  const totalMesesValor = anio * 12 + mes;
		  const totalMesesActual = anioActual * 12 + mesActual;

		  const diferencia = totalMesesActual - totalMesesValor;
