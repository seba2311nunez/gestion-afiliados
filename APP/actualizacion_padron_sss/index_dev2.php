...
<html>
	...
	<body>
		...
		<div id="modalDiscapacitados" class="modal fade" role="dialog">
		  <div class="modal-dialog modal-lg" style="width: 1300px;">
		    <div class="modal-content">
		      <div class="modal-header" style="background:#d9534f; color:#fff;">
		        <button type="button" class="close" data-dismiss="modal" style="color:#fff; opacity:1;">&times;</button>
		        <h4 class="modal-title">
		        	Afiliados con discapacidad enviados en la presentación
		        </h4>
		        <p style="margin:0;">Periodo: <b><span id="s_periodo_discapacitados"></span></b> | ID Lote: <b><span id="s_lote_discapacitados"></span></b></p>
		      </div>
		      <div class="modal-body">
		      	<div class="row" style="padding-left: 10px; padding-bottom: 10px;">
		      		<input type="hidden" id="id_lote_discapacitados">

		      		<a id="btnExportarExcelDiscapacitados" target="_blank" class="btn btn-danger">
					    <i class="fa fa-file-excel-o"></i> Exportar discapacitados a Excel
					</a>
		      	</div>

				<div id="divListDiscapacitados" class="table-responsive"></div>
		      </div>
		      <div class="modal-footer">
		        <button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
		      </div>
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				let filtroIncapacidadPresentacion = ''; // filtro exacto para botón DES/INC
				...
				function genera_tabla_discapacitados(){
					$('#divListDiscapacitados').html(
						"<table id='tabListaDiscapacitados' class='table table-condensed table-striped'>"
							+"<thead>"
								+"<tr class='danger'>"
									+"<th>#</th>"
									+"<th></th>"
									+"<th>CUIL presentado</th>"
									+"<th>Tipo mov. presentado</th>"
									+"<th>CUIL titular</th>"
									+"<th>Parentesco</th>"
									+"<th>CUIL discapacitado</th>"
									+"<th>DNI</th>"
				...
				function llena_tabla_discapacitados(id_lote) {
				    if ($.fn.DataTable.isDataTable('#tabListaDiscapacitados')) {
				        $('#tabListaDiscapacitados').DataTable().destroy();
				    }

				    $('#tabListaDiscapacitados').DataTable({
				        serverSide: true,
				        processing: true,
				        ajax: {
				            url: 'ajax_dev2.php',
				            type: 'GET',
				            data: {
				                parametro: 'lst_discapacitados_presentacion_ssp',
				                id_lote: id_lote
				            }
				        },
				        ...
				        language: {
				            ...
				            zeroRecords: "No se encontraron discapacitados en este lote",
				    ...
				    tablaAfiliados = $('#tabListaAfilPrestacion').DataTable({
				        ...
				        ajax: {
				            ...
				            data: function(d){
				                ...
				                d.id_lote = id_lote;
				                d.filtro_incapacidad = filtroIncapacidadPresentacion;
				            }
				        },
				        dom: '<"row mb-2"<"col-sm-6"l><"col-sm-6 text-right"Bf>>rt<"row mt-2"<"col-sm-6"i><"col-sm-6"p>>',
				        buttons: [
				...
				$('#tabListado tbody').on('click','.btnExportar',function(){
					...
					var url = "ajax_dev2.php?parametro=CrearArchivo&id_lote="+id_lote+"&periodo="+periodo+"&fecha_cierre="+fecha_cierre;
					abrirEnPestana(url);
				});
				...
				$(document).on('click', '.btnVerDiscapacitadosResumen', function(e){
					e.preventDefault();

					filtroIncapacidadPresentacion = '01';

					if(tablaAfiliados){
						tablaAfiliados.search('').draw();
					}
				});
				...
				$(document).on('click','#btnExportarExcel',function(){
					var url = "ajax_dev2.php?parametro=exportar_afiliados_excel&id_lote="+$("#id_lote").val();
					abrirEnPestana(url);
				});

				$(document).on('click','#btnExportarExcelDiscapacitados',function(){
					var url = "ajax_dev2.php?parametro=exportar_discapacitados_excel&id_lote="+$("#id_lote_discapacitados").val();
					abrirEnPestana(url);
				});
			...
			function cargarResumenAgrupado(id_lote) {
			    $.getJSON("ajax_dev2.php", { parametro: "resumen_tbt_y_errores", id_lote: id_lote }, function (data) {
				    const resumenTBT = data.resumen_tbt || [];
				    const resumenErrores = data.resumen_errores || [];
				    const resumenDiscapacitados = parseInt(data.resumen_discapacitados || 0, 10);
				    let yaMostroDes = false;
					...
					resumenTBT.forEach(item => {
					    ...
					    if (tbt == 'DES') {
					        yaMostroDes = true;
					        htmlResumen += `<a href='#' class='btn btn-sm btn-danger btnVerDiscapacitadosResumen' data-tbt='DES'>DES: ${resumenDiscapacitados}</a> `;
					    } else {
					        htmlResumen += `<a href='#' class='btn btn-sm btn-info filtro-tbt' data-tbt='${tbt}'>${tbt}: ${item.cantidad}</a> `;
					    }
					});

					if (!yaMostroDes && resumenDiscapacitados > 0) {
					    htmlResumen += `<a href='#' class='btn btn-sm btn-danger btnVerDiscapacitadosResumen' data-tbt='DES'>DES: ${resumenDiscapacitados}</a> `;
					}
					htmlResumen += "</div></div>";
